...
class Config:
    ...
    IMG_DIR: str = r"/home/yanghaotian/server_data/yanghaotian/data/ubc_frames/frame1"
...
class MaskBuilder:
    ...
    def build_person_mask(
        ...
        H, W = img_shape_hw
        results = self.models.yolo(img_path, verbose=False)
        result = results[0]
...
class PatternDetector:
    ...
    def detect_pattern_region(
        ...
        # Determine threshold to include "entire dense range" (User request)
        mean_val = float(vals.mean())
        std_val = float(vals.std())
        threshold = max(10, mean_val + 0.1 * std_val)
        ...
        adaptive_min = max(params.MIN_PATTERN_PIXELS, int(cloth_area * 0.05))
        if pattern_area < adaptive_min:
            return None

        # ===== 仅保留最复杂的单个连通区域 =====
        pattern_mask = self._keep_most_complex_component(pattern_mask, density)

        pattern_cut = cv2.bitwise_and(img_bgr, img_bgr, mask=pattern_mask)
        return pattern_mask, pattern_cut

    def _keep_most_complex_component(
        self, mask: np.ndarray, density: np.ndarray
    ) -> np.ndarray:
        """在所有连通区域中，仅保留平均边缘密度最高的单个区域"""
        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
            mask, connectivity=8
        )
        if num_labels <= 2:
        ...
        for i in range(1, num_labels):
            ...
            # 用该区域内的平均 density 作为复杂度得分
            region_vals = density[component_mask]
            if region_vals.size > 0:
                score = float(region_vals.mean())
            else:
                score = 0.0
            if score > best_score:
                best_score = score
...
class Visualizer:
    ...
    def save_summary_2x3(
        ...
        """使用 cv2 拼接 2x3 grid，每个 cell 保持原图尺寸 (H, W)，图片之间有间隔"""
        H, W = img_bgr.shape[:2]
        ...
        # 将所有面板统一为 BGR 3通道、(H, W) 尺寸
        cells = []
        for panel in panels:
            p = Visualizer._to_bgr3(panel)
            ph, pw = p.shape[:2]
            if (ph, pw) != (H, W):
                p = cv2.resize(p, (W, H), interpolation=cv2.INTER_LANCZOS4)
            cells.append(p)
        ...
        canvas = np.full((canvas_h, canvas_w, 3), gap_color, dtype=np.uint8)

        # 将每个 cell 放到画布上
        for idx, cell in enumerate(cells):
            r = idx // cols  # 行号 0 or 1
            c = idx % cols   # 列号 0, 1, 2
            y = r * (H + gap)
            x = c * (W + gap)
...
class ComplexAreaIdentifier:
    ...
    def process_frame(
        ...
        if final_cut is not None:
            ...
            cloth_cut = cv2.bitwise_and(frame_bgr, frame_bgr, mask=cloth_mask)
            
            # 保存 2x3 summary（使用 cv2，保持原图尺寸）
            ...
            # # 单独保存 region_cut 图片（保持原图尺寸 HxW）
            # region_cut_path = save_path.replace('.png', '_region_cut.png').replace('.jpg', '_region_cut.jpg')
            # cv2.imwrite(region_cut_path, final_cut)
            cv2.imwrite(save_path, cloth_cut)
    ...
    def process_frame_debug(
        ...
        # cv2.imwrite(os.path.join(save_dir, f"{file_prefix}_result.png"), pattern_cut)
        
        # 4. 使用 Visualizer 保存 2x3 summary（保持原图尺寸）
        cloth_cut_dbg = cv2.bitwise_and(frame_bgr, frame_bgr, mask=cloth_mask)
        Visualizer.save_summary_2x3(
            frame_bgr,
            cloth_cut_dbg,
            edges,
            ...
            pattern_mask,
            pattern_cut,
            os.path.join(save_dir, f"{file_prefix}_combined.png"),
            has_complex_region=(not use_cloth_mask)
        )
...
def main():
    ...
    # ============================================================
    # 新增逻辑：如果开启了目录模式，直接处理 IMG_DIR 下的图片
    # ============================================================
    if config.USE_DIR_MODE:
        print(f"\n{'='*60}")
        print(f"[DIR MODE] Processing images from directory: {config.IMG_DIR}")
        print(f"{'='*60}")

        img_paths = list_images_in_dir(config.IMG_DIR)
        if not img_paths:
            print(f"[ERROR] No valid images found in {config.IMG_DIR}")
            return

        # 在目录模式下兼容 auto_iterative
        if mode == "auto_iterative":
            print(f"\n[AUTO-ITERATIVE] Collecting calibration frames from {config.IMG_DIR}...")
            calib_pairs = []
            sample_paths = img_paths[:config.ITER_MAX_IMAGES] # 取前 N 张图做校准
        ...
        for idx, img_path in enumerate(img_paths):
            print(f"Processing [{idx+1}/{len(img_paths)}]: {os.path.basename(img_path)}")
            ...
            try:
                ...
                if not success:
                    print(f"Failed to extract complex region for {img_filename}")
            except Exception as e:
                print(f"Error processing {img_filename}: {e}")

        print(f"\n[DONE] Directory processing complete. Output: {run_output_dir}")
        return  # 执行完目录模式后直接退出，不执行后续的 JSON 逻辑


    # ============================================================
    # 原有的逻辑：如果 USE_DIR_MODE 为 False，则走 JSON 视频帧提取流程
    # ============================================================
    try:
    ...
    all_datasets: List[Tuple[str, List[dict]]] = []
    ...
    if mode == "auto_iterative":
        ...
        for json_path, items in all_datasets:
            ...
            count = 0
            for item in items:
                if len(calib_pairs) >= max_calib: break
                if count >= per_ds: break
                video_rel = item.get('video_path')
                idx = item.get('idx')
                if not video_rel or idx is None: continue
    ...
    for json_path, items in all_datasets:
        ...
        for item in items:
            ...
            if not video_rel_path or idx is None or not img_rel_path: continue
