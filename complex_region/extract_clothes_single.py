...
def main_image():
    """
    单张图片模式：
    输入一张图片路径，提取衣物区域（或复杂区域）
    """

    config = CONFIG
    print("Initializing Complex Area Identifier (IMAGE MODE)...")

    # 1️⃣ 初始化参数（可改成 load_json）
    params = Params().normalize()
    identifier = ComplexAreaIdentifier(config)
    identifier._initialize(params)

    # 2️⃣ 输入图片路径（修改成你的路径）
    image_path = "/home/yanghaotian/server_data/yanghaotian/data/applied_dataset/ref/MEN-Pants-id_00001207-07_4_full.png"   # ← 改成你的图片路径
    output_path = "/home/yanghaotian/server_data/yanghaotian/data/test_comp/pants2_clothes"
    ...
    success = identifier.process_frame(img, output_path)

    if success:
        print(f"[DONE] Saved to {output_path}")
    else:
        print("Failed to extract cloth region.")
