...
def main():
    ...
    import glob
    meta_json_paths = glob.glob(os.path.join(script_dir, "..", "meta_comp1", "*.json")) + \
                      glob.glob(os.path.join(script_dir, "..", "meta_comp2", "*.json")) + \
                      glob.glob(os.path.join(script_dir, "..", "meta_comp3", "*.json"))

    for mp in meta_json_paths:
        if not os.path.exists(mp):
            print(f"[WARN] Metadata not found: {mp}, skipping.")
            continue
        try:
            with open(mp, 'r') as f:
                items = json.load(f)
    ...
    def resolve_video_path(video_rel_path: str, json_dir: str) -> Optional[str]:
        if not video_rel_path.startswith("/"):
            video_rel_path_adjusted = "../" + video_rel_path
        else:
            video_rel_path_adjusted = video_rel_path

        vp = os.path.normpath(os.path.join(json_dir, video_rel_path_adjusted))
        ...
        for item in items:
            ...
            try:
                if img_rel_path.startswith("/"):
                    save_path = img_rel_path
                else:
                    # 按照用户要求，给每个路径最开头再加上一个../
                    target_img_path = "../" + img_rel_path
                    save_path = os.path.normpath(os.path.join(script_dir, target_img_path))
