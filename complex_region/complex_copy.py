...
class PatternDetector:
    ...
    def _remove_small_components(mask: np.ndarray, min_area: int = 500) -> np.ndarray:
        ...
        2. 相对面积阈值：小于最大连通区域 10% 的区域移除（防止碎片残留）
        """
        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
            mask, connectivity=8
        )
...
class ComplexAreaIdentifier:
    ...
    def process_frame(
        ...
        if cloth_area > 0:
            inner_mask = self.pattern_detector._create_inner_mask(cloth_mask, frame_bgr)
    ...
    def process_frame_debug(
        ...
        if cloth_area > 0:
            ...
            _, density_raw = self.img_processor.compute_edge_density(
                ...
                self.pattern_detector._create_inner_mask(cloth_mask, frame_bgr)
            )
