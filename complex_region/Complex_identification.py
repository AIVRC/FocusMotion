...
class Visualizer:
    ...
    def save_summary_2x3(
        ...
        if has_complex_region:
            ...
            label_text = "X No Complex Region (Fallback to Cloth)"
...
def main():
    ...
    # ========== 2. Skip Metadata Loading (Folder Mode) ==========
    # Since we are processing a specific folder, we don't need metadata JSONs.
    all_datasets: List[Tuple[str, List[dict]]] = []
    # (Original metadata loading block removed to prevent early exit)
    ...
    for json_rel_path in config.META_JSON_PATHS:
        json_path = os.path.normpath(os.path.join(script_dir, json_rel_path))
        ...
        print(f"\n{'='*60}")
        print(f"[JSON] Processing: {json_path} ({len(items)} entries)")
        print(f"{'='*60}")

        for i, item in enumerate(items):
            img_rel = item.get("img_path")
            if not img_rel:
                print(f"  [{i+1}/{len(items)}] Skipping entry: no img_path field")
                continue

            # img_path 以 "./" 开头，相对于项目根目录
            img_path = os.path.normpath(os.path.join(project_root, img_rel))

            if not os.path.exists(img_path):
                print(f"  [{i+1}/{len(items)}] Image not found: {img_path}")
                total_failed += 1
                continue

            frame = cv2.imread(img_path)
            if frame is None:
                print(f"  [{i+1}/{len(items)}] Could not read image: {img_path}")
                total_failed += 1
                continue
            ...
            try:
                ...
                if success:
                    total_processed += 1
                    print(f"  [{i+1}/{len(items)}] OK: {img_filename}")
                else:
                    total_failed += 1
                    print(f"  [{i+1}/{len(items)}] Failed: {img_filename}")
            except Exception as e:
                total_failed += 1
                print(f"  [{i+1}/{len(items)}] Error processing {img_filename}: {e}")

    print(f"\n[DONE] Processed {total_processed} images, {total_failed} failed. Output: {run_output_dir}")
