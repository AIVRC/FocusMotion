...
from src.models.gated_cross_attention import GatedCrossAttention
...
#         )
#         return self.norm(
#             query + torch.sigmoid(self.gate) * attn_out
#         )
...
class Net(nn.Module):
    def __init__(
        ...
        self.clip_fusion = clip_fusion # <-- Use the passed instance
    ...
    def forward(
        ...
        ref_image_latents,
        clip_image_embeds,
        clip_complex_embeds, #add clip fused embeds
        pose_img,
        ...
        fused_prompt_embeds = self.clip_fusion(
            clip_image_embeds,
            clip_complex_embeds,
        )
        ...
        if not uncond_fwd:
            ...
            self.reference_unet(
                ...
                encoder_hidden_states2=fused_prompt_embeds,
                return_dict=False,
            )
...
def main(cfg):
    ...
    # 1. Instantiate the clip_fusion model
    clip_fusion = GatedCrossAttention(
        dim=768, 
        num_heads=8
    ).to(device="cuda")
    ...
    if cfg.load_pth:
        ...
        clip_fusion.load_state_dict(
            torch.load(cfg.clip_fusion_path, map_location="cpu"),
        )
    ...
    clip_fusion.requires_grad_(True) # <-- Set requires_grad appropriately
    ...
    net = Net(
        ...
        reference_control_writer,
        reference_control_reader,
        clip_fusion, # <-- Pass the loaded module here
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        #     print("Gate value:",
        #         torch.sigmoid(net.clip_fusion.gate).item())
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(net):
                ...
                clip_cmplx_list = []
                #add key clip_cmplx
                for batch_idx, (ref_img, clip_img, clip_cmplx) in enumerate(
                    zip(
                        batch["ref_img"],
                        batch["clip_images"],
                        batch["clip_cmplx"],
                    ...
                    if uncond_fwd:
                        ...
                        #append to clip_cmplx_list
                        clip_cmplx_list.append(torch.zeros_like(clip_cmplx))
                    else:
                        clip_image_list.append(clip_img)
                        #append to clip_cmplx_list
                        clip_cmplx_list.append(clip_cmplx)
                ...
                with torch.no_grad():
                    ...
                    #confirm
                    # print("clip_dim",image_prompt_embeds.shape)
                    #add clip_cmplx_embeds
                    clip_cmplx = torch.stack(clip_cmplx_list, dim=0).to(
                        dtype=image_enc.dtype, device=image_enc.device
                    )
                    clip_cmplx_embeds = image_enc(
                        clip_cmplx.to("cuda", dtype=weight_dtype)
                    ).image_embeds
                    complex_prompt_embeds = clip_cmplx_embeds.unsqueeze(1)
                ...
                model_pred = net(
                    ...
                    ref_image_latents,
                    image_prompt_embeds,
                    complex_prompt_embeds, #add complex prompt embeds
                    tgt_pose_img,
                    uncond_fwd,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train_stage_1_comp.yaml")
