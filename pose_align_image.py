...
def run_align_video_with_filterPose_translate_smooth(args, img_name):
    ...
    filepath = vidfn
    # filepath = "/root/MusePose/assets/fashion/move/7.jpg"
    pose_image = cv2.imread(filepath) 
    ...
    for i in range(1):       
        video_frame_buffer.append(pose_image)


       
        # estimate scale parameters by the 1st frame in the video
        if True:
            output_1st_img, pose_1st_img = detector(pose_image, args.detect_resolution, args.image_resolution, output_type='cv2', return_pose_dict=True)
            ...
            align_args["scale_neck"] = (dist_ref_img / dist_1st_img) * 1  # align / pose = ref / 1st
            ...
            align_args["scale_shoulder"] = (dist_ref_img / dist_1st_img) * 1
            ...
            align_args["scale_arm_upper"] = (s1+s2)/2 * 1 # 1.548
            ...
            s2 = dist_ref_img / dist_1st_img
            align_args["scale_arm_lower"] = ((s1+s2)/2) * 1
            ...
            for i in range (10): 
                if dist_1st_img[i] != 0:
                    ...
                    count = count + 1
            if count!=0:
                align_args["scale_hand"] = ((ratio/count+align_args["scale_arm_upper"]+align_args["scale_arm_lower"])/3) * 1
            else:
                align_args["scale_hand"] = ((align_args["scale_arm_upper"]+align_args["scale_arm_lower"])/2) * 1
            ...
            align_args["scale_body_len"]= (dist_ref_img / dist_1st_img) * 1
            ...
            s2 = dist_ref_img / dist_1st_img
            align_args["scale_leg_upper"] = ((s1+s2)/2) * 1
            ...
            s2 = dist_ref_img / dist_1st_img
            align_args["scale_leg_lower"] = ((s1+s2)/2) * 1
            ...
            # pose align
            pose_img, pose_ori = detector(pose_image, args.detect_resolution, args.image_resolution, output_type='cv2', return_pose_dict=True)
            video_pose_buffer.append(pose_img)
            pose_align = align_img(pose_image, pose_ori, align_args, args.detect_resolution, args.image_resolution)
    ...
    # pose_dir = "/root/MusePose/assets/fashion/pose"
    # pose_dir = outfn
    # os.makedirs(pose_dir, exist_ok=True)
    # move_name = os.path.basename(filepath).split('.')[0]
    if(len(pose_list)==1):
        print('pose align done')
        for idx, detected_map in enumerate(result_demo):
            cv2.imwrite(outfn, detected_map)
            print(outfn)
        for idx, detected_map in enumerate(result_pose_only):
            cv2.imwrite(args.outfn_align_pose_video, detected_map)
            print(args.outfn_align_pose_video)
        if args.if_edit:
            # 身体姿态生成
            pose_body = pose_t["bodies"]["candidate"].copy()
            pose_body[..., 0] = pose_body[..., 0]*float(W1)
            pose_body[..., 1] = pose_body[..., 1]*float(H)
            pose_keypoints_2d = []
            for i in pose_body:
                for j in i: 
                    pose_keypoints_2d = pose_keypoints_2d + [float(j)] 
                pose_keypoints_2d = pose_keypoints_2d + [1]
            # print(pose_keypoints_2d)
            # 右手姿态生成
            pose_right_hands = pose_t["hands"][0].copy()
            pose_right_hands[..., 0] = pose_right_hands[..., 0]*float(W1)
            pose_right_hands[..., 1] = pose_right_hands[..., 1]*float(H)
            hand_right_keypoints_2d = []
            for i in pose_right_hands:
                for j in i: 
                    hand_right_keypoints_2d = hand_right_keypoints_2d + [float(j)] 
                hand_right_keypoints_2d = hand_right_keypoints_2d + [1]
            # print(hand_right_keypoints_2d)
            # 左手姿态生成
            pose_left_hands = pose_t["hands"][1].copy()
            pose_left_hands[..., 0] = pose_left_hands[..., 0]*float(W1)
            pose_left_hands[..., 1] = pose_left_hands[..., 1]*float(H)
            hand_left_keypoints_2d = []
            for i in pose_left_hands:
                for j in i: 
                    hand_left_keypoints_2d = hand_left_keypoints_2d + [float(j)]
                hand_left_keypoints_2d = hand_left_keypoints_2d + [1]
            # print(hand_left_keypoints_2d)
            dict_pose = {"people":[{"pose_keypoints_2d":pose_keypoints_2d, 
                                    "hand_right_keypoints_2d":hand_right_keypoints_2d,
                                    "hand_left_keypoints_2d":hand_left_keypoints_2d}],
                                    "animals":[],"canvas_width":W1,"canvas_height":H}
            import json
            with open("./edit_posealign.json", "w") as outfiles:  
                json.dump(dict_pose, outfiles, indent = 4)
            print('骨骼json地址在: ./edit_posealign.json')
    else:
        print('there is not a image')
...
def main():
    ...
    parser.add_argument('--imgfn_refer', type=str, default="/root/MusePose/assets/images/mote/tiaoshou/2.jpg", help='refer image path')
    # vidfn这个输入动作图片地址。
    parser.add_argument('--vidfn', type=str, default="/home/yanghaotian/server_data/yanghaotian/test/MusePose/assets/images/ref.png", help='Input video path')
    ...
    parser.add_argument('--if_edit', type=str, default=True, help='if edit the pose after pose align')
    ...
    if args.outfn_align_pose_video is None:
        args.outfn_align_pose_video = "./assets/poses/align/img_{}_video_{}.png".format(img_name, video_name)
    if args.outfn is None:
        args.outfn = "./assets/poses/align_demo/img_{}_video_{}.png".format(img_name, video_name)

    run_align_video_with_filterPose_translate_smooth(args, img_name)
