...
from musepose.models.unet_2d_condition_comp import UNet2DConditionModel
from musepose.models.unet_3d_comp import UNet3DConditionModel
...
def parse_args():
    ...
    parser.add_argument("--config",default="./configs/test_stage_1_comp.yaml")
...
def main():
    ...
    #load cross attention fusion module
    clip_fusion = GatedCrossAttention(dim=768).to(
    dtype=weight_dtype, device="cuda"
    )

    clip_fusion.load_state_dict(
        torch.load(config.clip_fusion_path, map_location="cpu")
    )
    clip_fusion.eval()
    ...
    save_dir_grid = Path(f"./output/image-{date_str}/{save_dir_name}/grid")
    save_dir_res = Path(f"./output/image-{date_str}/{save_dir_name}/res")
    save_dir_res.mkdir(exist_ok=True, parents=True)
    save_dir_grid.mkdir(exist_ok=True, parents=True)

    def handle_single(ref_image_path, pose_path, complex_image_path, seed):
        ...
        #add complex image
        complex_image_pil = Image.open(complex_image_path).convert("RGB")
        ...
        for img in imgs:
            ...
            img.save(os.path.join(save_dir_res, f"res_{ref_name}_{pose_name}_{args.cfg}_{seed}.jpg"))
        ...
        image_grid.save(os.path.join(save_dir_grid, f"grid_{ref_name}_{pose_name}_{args.cfg}_{seed}.jpg"))
    ...
    for case in config["test_cases"]:
        ref_image_path = case["ref"]
        pose_image_path = case["pose"]
        complex_image_path = case["complex"]
